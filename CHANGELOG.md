# SimBill v1.17.0 — Reconnect Otomatis (Hotspot Stale + Deteksi Reboot MikroTik)

Tanggal: 2026-07-20

## Ringkasan
Dua masalah reconnect yang selama ini butuh intervensi manual kini otomatis:

1. **Hotspot: user menjauh/putus lalu balik ditolak "already login".**
   Saat user hotspot kehilangan sinyal / menjauh, MikroTik kadang belum kirim
   Acct-Stop sehingga sesi nyangkut di radacct. Karena Simultaneous-Use=1
   (1 voucher 1 device), user ditolak saat balik. Sekarang cron sinkron membaca
   `/ip/hotspot/active` MikroTik dan menutup sesi radacct yang user-nya sudah
   tidak online → user balik langsung bisa login.

2. **PPPoE: MikroTik reboot → sesi lama nyangkut → auth failed massal.**
   Setelah router reboot, sesi lama di radacct tak sempat di-stop → RADIUS
   menolak / user gagal konek, harus dibersihkan manual. Sekarang cron mendeteksi
   reboot via uptime yang turun, lalu menutup sesi NAS itu → pelanggan konek
   otomatis tanpa intervensi.

## Perubahan
- **BARU** `backend/services/sinkron-hotspot.js` — modul sinkron hotspot stale.
- **BARU** `backend/services/deteksi-reboot.js` — modul deteksi reboot via uptime.
- **BARU** `backend/jalan-sinkron-hotspot.js` — runner cron (2 menit).
- **BARU** `backend/jalan-deteksi-reboot.js` — runner cron (2 menit).
- **DB** kolom baru `router.last_uptime_sec` (migrasi otomatis di update.sh,
  idempoten `ADD COLUMN IF NOT EXISTS`).
- **update.sh** fungsi `migrasi_last_uptime` + `setup_cron_reconnect`
  (pasang 2 cron idempoten, deteksi path node otomatis).

## Safeguard (penting)
- **Per-router**: router gagal dibaca (timeout) → sesi NAS itu di-SKIP, router
  lain tetap jalan. Cegah salah-stop saat data tak lengkap.
- **Hotspot**: idle minimal 3 menit sebelum dianggap stale (hindari race user
  yang baru login); rasio stale per-router >60% → skip (curiga matching salah).
- **Reboot**: first-run (uptime baseline belum ada) → simpan saja, tak stop;
  reboot = uptime turun >60 detik (bukan jitter); gagal baca uptime → skip.
- Semua penutupan sesi = `acctstoptime=NOW()` (histori tetap), BUKAN delete.
- Runner: crash → exit 0 (tak spam), timeout 45–60 detik.

## Catatan
- Cache NAS (rencana v1.16.2) TIDAK disertakan: masalah /nas lambat sudah tuntas
  via kenaikan limit file descriptor (LimitNOFILE) di sisi sistem, sehingga cache
  tidak diperlukan. radius.js tidak diubah di rilis ini.
- Hotspot matching pakai field `user` dari `/ip/hotspot/active` (= username
  voucher, cocok 1:1 dgn radacct.username). PPPoE TIDAK pakai matching username
  (arsitektur PPPoE tersebar via VLAN/OLT membuat /ppp/active tak lengkap) —
  PPPoE ditangani lewat deteksi reboot (uptime), bukan matching sesi.

# CHANGELOG v1.16.0 — Fitur PPN (Pajak Pertambahan Nilai)

Fitur baru: PPN per-pelanggan + PPN global, model **eksklusif** (ditambah di atas
harga). **Default OFF** → instalasi lama & invoice lama TIDAK berubah sampai diaktifkan.

## Model
- **PPN Global** (Pengaturan Sistem): master switch `PPN Aktif` + `PPN Global (%)`.
  Nonaktif = tak ada PPN untuk siapa pun.
- **PPN per-pelanggan** (form Tambah/Edit Pelanggan):
  - Dikosongkan → **ikut PPN global**
  - Diisi `0` → **bebas PPN** (eksplisit)
  - Diisi angka → **tarif khusus** pelanggan itu
- **Eksklusif**: `total = harga + (harga × PPN%)`. DPP = harga.
- **Forward-only**: hanya invoice BARU. Invoice lama tak diutak.

## Backend
- `services/ppn.js` (baru) — perhitungan PPN terpusat (satu sumber kebenaran).
- Migrasi `server.js`: kolom `pelanggan.ppn_persen`, `invoice.dpp/ppn_persen/ppn_nominal`,
  setting `ppn_aktif`(=0) + `ppn_persen`(=11). Idempoten, default OFF.
- Semua 8 titik invoice langganan pakai helper + kirim total PPN-included ke payment
  gateway (`cron.js` ×2, `invoice.js` manual+batch, `pelanggan.js` prorata paid/unpaid,
  `client.js` portal). Invoice manual: nominal admin = DPP, PPN ditambah di atasnya.
- `pelanggan.js` create/edit terima `ppn_persen` (edit pertahankan nilai lama bila
  field tak dikirim → aman untuk edit parsial).
- `whatsapp.js` (aditif): placeholder `{rincian_ppn}` yang menyembunyikan diri saat
  PPN=0. Placeholder `{ppn}`/`{vat_percent}`/`{dpp}` diisi nilai nyata.

## Frontend (`admin.html`)
- Pengaturan Sistem: section PPN (toggle Aktif + PPN Global %).
- Form Tambah & Edit Pelanggan: field PPN (%) opsional.
- Tampilan invoice: rincian Sub Total (DPP) + baris PPN + Total (hanya saat PPN>0).

## Contoh (harga 100.000, PPN global 11% ON)
- Pelanggan kosong → 100.000 + 11.000 = **111.000**
- Pelanggan diisi 0 → **100.000** (bebas)
- Pelanggan diisi 5 → 100.000 + 5.000 = **105.000**
- PPN Aktif = OFF → semua **100.000**

## Voucher TIDAK terpengaruh (keputusan: hanya invoice langganan pelanggan).

## Catatan file sensitif
- `server.js` = FILE INTI (migrasi jalan saat boot).
- `services/whatsapp.js` = RAWAN DRIFT — perubahan sudah diverifikasi **aditif** (diff).
- `routes/client.js` juga membawa hardening #3 v1.15.2 (superset, aman).
