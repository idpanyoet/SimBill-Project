...
if [ -d "$BACKEND_DIR" ]; then
    ...
    ensure_dep ssh2
    ensure_dep net-snmp
    ensure_dep node-routeros    # API MikroTik (baca /ppp active, putus sesi nyangkut) — v1.12.0
fi
...
# 4b) Selaraskan versi yang ditampilkan panel dengan rilis terbaru di repo publik.
#     Baca file VERSION MENTAH dari repo publik (TANPA token, TANPA rate-limit
#     GitHub API). Repo publik → tak perlu github_token sama sekali.
REPO_OWNER="idpanyoet"
REPO_NAME="SimBill-Project"
c_info "Menyelaraskan versi ..."
LATEST_TAG=$(wget -qO- "https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/${BRANCH}/VERSION" 2>/dev/null | tr -d '[:space:]')
# fallback lama (API rilis) hanya bila raw gagal — tetap non-fatal, tanpa token wajib:
...
# ============================================================================
#  WAHA (WhatsApp HTTP API, Docker) — OPT-IN, non-fatal, "install != pakai".
#  update.sh TIDAK memasang Docker & TIDAK menarik image besar (biar update
#  cepat & aman). Tugasnya: PASTIKAN container WAHA yang sudah ada tetap hidup,
#  dan bila image sudah ada + di-opt-in, jalankan container (sekali).
#  Provisioning penuh (install Docker + tarik image) lewat 'setup-waha.sh'.
#  Opt-in: file /opt/waha/ENABLE ADA, atau container 'waha' sudah pernah dibuat.
#  Port default 127.0.0.1:3100 (JANGAN 3000 = SimBill). Aktivasi tetap di panel.
# ============================================================================
setup_waha() {
    local WAHA_DIR="/opt/waha"
    local WAHA_NAME="waha"
    local WAHA_PORT="3100"
    local WAHA_IMG="devlikeapro/waha"

    # Opt-in: hanya urus WAHA bila ada penanda ENABLE atau container sudah pernah dibuat.
    local HAS_CONTAINER=0
    if command -v docker >/dev/null 2>&1 && docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx "$WAHA_NAME"; then
        HAS_CONTAINER=1
    fi
    if [ "$HAS_CONTAINER" -eq 0 ] && [ ! -f "${WAHA_DIR}/ENABLE" ]; then
        return 0   # tidak di-opt-in → diam total
    fi

    # Docker WAJIB, tapi TIDAK diinstal otomatis oleh update (terlalu invasif).
    if ! command -v docker >/dev/null 2>&1; then
        c_info "WAHA: Docker belum terpasang — jalankan sekali: bash ${BACKEND_DIR%/backend}/setup-waha.sh"
        return 0
    fi
    if ! docker info >/dev/null 2>&1; then
        systemctl start docker >/dev/null 2>&1 || true
        docker info >/dev/null 2>&1 || { c_info "WAHA: daemon Docker tidak jalan (dilewati)"; return 0; }
    fi

    # Container sudah ada → pastikan berjalan (idempotent).
    if [ "$HAS_CONTAINER" -eq 1 ]; then
        if docker ps --format '{{.Names}}' | grep -qx "$WAHA_NAME"; then
            c_ok "WAHA: container aktif (127.0.0.1:${WAHA_PORT})"
        else
            docker start "$WAHA_NAME" >/dev/null 2>&1 \
                && c_ok "WAHA: container dijalankan ulang (127.0.0.1:${WAHA_PORT})" \
                || c_info "WAHA: gagal start container (cek: docker logs ${WAHA_NAME})"
        fi
        return 0
    fi

    # Belum ada container + opt-in. Hanya provision bila IMAGE sudah ada
    # (hindari tarik image besar di tengah update). Kalau belum, arahkan ke setup-waha.sh.
    if ! docker image inspect "$WAHA_IMG" >/dev/null 2>&1; then
        c_info "WAHA: image belum ada — jalankan sekali: bash ${BACKEND_DIR%/backend}/setup-waha.sh (unduh image + start)"
        return 0
    fi
    mkdir -p "${WAHA_DIR}/sessions"
    local HOOK_ARGS=""
    if [ -f "${WAHA_DIR}/.env" ] && grep -q '^HOOK_URL=' "${WAHA_DIR}/.env"; then
        local HU; HU="$(grep '^HOOK_URL=' "${WAHA_DIR}/.env" | cut -d= -f2-)"
        [ -n "$HU" ] && HOOK_ARGS="-e WHATSAPP_HOOK_URL=${HU} -e WHATSAPP_HOOK_EVENTS=message"
    fi
    if docker run -d --restart always --name "$WAHA_NAME" \
         -p 127.0.0.1:${WAHA_PORT}:3000 \
         -e WHATSAPP_DEFAULT_ENGINE=NOWEB \
         -v "${WAHA_DIR}/sessions:/app/.sessions" \
         ${HOOK_ARGS} \
         "$WAHA_IMG" >/dev/null 2>&1; then
        c_ok "WAHA: container aktif (127.0.0.1:${WAHA_PORT}). Aktifkan di panel: Setting > WhatsApp > WAHA > Simpan, lalu Scan QR."
    else
        c_info "WAHA: gagal start container (dilewati; cek: docker logs ${WAHA_NAME})"
    fi
}
setup_waha || c_info "WAHA dilewati (non-fatal)"

# Petunjuk ramah: kalau provider aktif = WAHA tapi container belum jalan, pelanggan

# ── Migrasi: kolom admin.permissions (RBAC) ─────────────────────────────────
# Kolom ini dibutuhkan middleware/perm.js & rbac.js. Instalasi lama TIDAK
# punya kolom ini -> panel akan error "Unknown column 'permissions'".
# Idempoten: aman dijalankan berkali-kali (IF NOT EXISTS).
migrasi_durasi() {
    command -v mysql >/dev/null 2>&1 || { c_info "Migrasi durasi dilewati (mysql tidak ada)"; return 0; }
    mysql -h"${DB_HOST:-127.0.0.1}" -P"${DB_PORT:-3306}" -u"${DB_USER:-root}" -p"${DB_PASS:-}" "${DB_NAME:-billing_radius}" <<'SQL' >/dev/null 2>&1
ALTER TABLE paket ADD COLUMN IF NOT EXISTS durasi_menit INT UNSIGNED NULL DEFAULT NULL AFTER satuan_masa;
SQL
    if [ $? -eq 0 ]; then
        c_ok "Kolom paket.durasi_menit siap (durasi hotspot)"
    else
        c_err "Gagal menambah kolom paket.durasi_menit. Jalankan manual:"
        c_err "  ALTER TABLE paket ADD COLUMN IF NOT EXISTS durasi_menit INT UNSIGNED NULL DEFAULT NULL AFTER satuan_masa;"
    fi
}

# Durasi hotspot butuh FreeRADIUS noresetcounter (sqlcounter). Kita TULIS config
# sendiri dgn dialect EKSPLISIT "mysql" — TIDAK symlink template default yang
# pakai ${modules.sql.dialect} (bisa gagal resolve bila sqlcounter di-load
# sebelum sql module → FreeRADIUS MATI → semua PPPoE putus).
# Tiap perubahan diverifikasi freeradius -CX; GAGAL → ROLLBACK PENUH (FreeRADIUS
# tetap jalan, durasi dilewati, bukan fatal).
setup_noresetcounter() {
    RADDIR="$(ls -d /etc/freeradius/*/ 2>/dev/null | head -1)"
    [ -z "$RADDIR" ] && { c_info "Durasi FreeRADIUS dilewati (FreeRADIUS tidak ada)"; return 0; }
    local ME="${RADDIR}mods-enabled/sqlcounter"
    local SITE="${RADDIR}sites-enabled/default"
    [ -f "$SITE" ] || { c_info "Durasi dilewati (sites-enabled/default tak ada)"; return 0; }
    command -v freeradius >/dev/null 2>&1 || { c_info "Durasi dilewati (freeradius CLI tak ada — tak bisa verifikasi aman)"; return 0; }

    # Idempoten: kalau config kita SUDAH terpasang (dialect eksplisit) & authorize
    # sudah panggil noresetcounter & config OK → skip.
    if [ -f "$ME" ] && grep -q 'dialect = mysql' "$ME" 2>/dev/null \
       && grep -qE '^\s*noresetcounter' "$SITE"; then
        c_info "Durasi noresetcounter sudah aktif (skip)"
        return 0
    fi

    # ── SNAPSHOT untuk rollback ──
    local SITE_BAK="${SITE}.bak-durasi-$(date +%s)"
    cp "$SITE" "$SITE_BAK" 2>/dev/null
    local ME_ADA=0; [ -e "$ME" ] && ME_ADA=1
    local ME_BAK=""
    if [ "$ME_ADA" = "1" ]; then ME_BAK="${ME}.bak-durasi-$(date +%s)"; cp -a "$ME" "$ME_BAK" 2>/dev/null; fi

    # 1) TULIS config noresetcounter (dialect eksplisit, query inline)
    cat > "$ME" <<'NORESET'
# noresetcounter — durasi hotspot SimBill (akumulasi total uptime voucher).
# dialect EKSPLISIT "mysql" (bukan ${modules.sql.dialect} yang bisa gagal resolve).
...
NORESET
    ...
    # 2) panggil noresetcounter di authorize (setelah -sql) bila belum
    if ! grep -qE '^\s*noresetcounter' "$SITE"; then
        local LN
        LN="$(grep -nE '^\s*-sql\s*$' "$SITE" | head -1 | cut -d: -f1)"
        if [ -n "$LN" ]; then
            local IND
            IND="$(sed -n "${LN}p" "$SITE" | sed -E 's/([[:space:]]*).*/\1/')"
            sed -i "${LN}a\\${IND}noresetcounter" "$SITE"
        fi
    fi

    # 3) VERIFIKASI -CX. GAGAL → ROLLBACK PENUH (FreeRADIUS tak disentuh).
    if freeradius -CX >/dev/null 2>&1; then
        ...
        if systemctl is-active --quiet freeradius; then
            c_ok "FreeRADIUS noresetcounter aktif (durasi hotspot)"
            rm -f "$SITE_BAK" "$ME_BAK"
        else
            c_err "FreeRADIUS gagal running setelah setup durasi — ROLLBACK."
            if [ "$ME_ADA" = "1" ] && [ -n "$ME_BAK" ]; then mv -f "$ME_BAK" "$ME"; else rm -f "$ME"; fi
            cp "$SITE_BAK" "$SITE" 2>/dev/null; rm -f "$SITE_BAK"
            systemctl restart freeradius >/dev/null 2>&1 || true
            c_info "Durasi dilewati (FreeRADIUS dipulihkan)."
        fi
    else
        c_err "Config FreeRADIUS TIDAK valid dgn sqlcounter di server ini — ROLLBACK (durasi dilewati)."
        if [ "$ME_ADA" = "1" ] && [ -n "$ME_BAK" ]; then mv -f "$ME_BAK" "$ME"; else rm -f "$ME"; fi
        cp "$SITE_BAK" "$SITE" 2>/dev/null; rm -f "$SITE_BAK"
        # FreeRADIUS TIDAK di-reload → config lama masih jalan → PPPoE AMAN.
        c_info "FreeRADIUS TIDAK disentuh (tetap jalan). PPPoE aman."
    fi
}

migrasi_permissions() {
    command -v mysql >/dev/null 2>&1 || { c_info "Migrasi permissions dilewati (mysql tidak ada)"; return 0; }
    mysql -h"${DB_HOST:-127.0.0.1}" -P"${DB_PORT:-3306}" -u"${DB_USER:-root}" -p"${DB_PASS:-}" "${DB_NAME:-billing_radius}" <<'SQL' >/dev/null 2>&1
ALTER TABLE admin ADD COLUMN IF NOT EXISTS permissions TEXT NULL AFTER role;
SQL
    if [ $? -eq 0 ]; then
        c_ok "Kolom admin.permissions siap (RBAC)"
    else
        c_err "Gagal menambah kolom admin.permissions — RBAC bisa error. Jalankan manual:"
        c_err "  ALTER TABLE admin ADD COLUMN IF NOT EXISTS permissions TEXT NULL AFTER role;"
    fi
}

# akan lihat error "tidak terhubung" di panel. Beri tahu cara pasang (bukan paksa).
_waha_hint() {
    [ -f "${BACKEND_DIR}/.env" ] || return 0
    command -v mysql >/dev/null 2>&1 || return 0
    set -a; . "${BACKEND_DIR}/.env" 2>/dev/null; set +a
    local prov
    prov="$(mysql -h"${DB_HOST:-127.0.0.1}" -P"${DB_PORT:-3306}" -u"${DB_USER:-root}" -p"${DB_PASS:-}" "${DB_NAME:-billing_radius}" -N -B -e "SELECT nilai FROM setting WHERE kunci='wa_provider' LIMIT 1" 2>/dev/null)"
    [ "$prov" = "waha" ] || return 0
    if ! (command -v docker >/dev/null 2>&1 && docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "waha"); then
        echo ""
        c_info "Provider WhatsApp aktif = WAHA, tapi WAHA belum berjalan."
        c_info "  → Pasang sekali:  sudo bash ${APP_DIR}/setup-waha.sh"
        c_info "  → Atau ganti provider di panel (Setting > WhatsApp) ke Fonnte/Mandiri."
    fi
}
_waha_hint || true
...
# ── v1.17.0: Reconnect otomatis (deteksi reboot + sinkron hotspot stale) ──
# Migrasi kolom router.last_uptime_sec (dipakai deteksi reboot MikroTik).
# Idempoten: ADD COLUMN IF NOT EXISTS.
migrasi_last_uptime() {
    command -v mysql >/dev/null 2>&1 || { c_info "Migrasi last_uptime dilewati (mysql tidak ada)"; return 0; }
    mysql -h"${DB_HOST:-127.0.0.1}" -P"${DB_PORT:-3306}" -u"${DB_USER:-root}" -p"${DB_PASS:-}" "${DB_NAME:-billing_radius}" <<'SQL' >/dev/null 2>&1
ALTER TABLE router ADD COLUMN IF NOT EXISTS last_uptime_sec BIGINT DEFAULT NULL;
SQL
    if [ $? -eq 0 ]; then
        c_ok "Kolom router.last_uptime_sec siap (deteksi reboot MikroTik)"
    else
        c_err "Gagal menambah kolom router.last_uptime_sec. Jalankan manual:"
        c_err "  ALTER TABLE router ADD COLUMN IF NOT EXISTS last_uptime_sec BIGINT DEFAULT NULL;"
    fi
}

# Pasang cron reconnect (sinkron-hotspot + deteksi-reboot, tiap 2 menit).
# Idempoten: buang baris lama dulu (cegah duplikat saat update ulang), tambah 1.
# Deteksi path node otomatis (JANGAN hardcode /usr/bin/node — beda tiap VPS).
setup_cron_reconnect() {
    local NODE_BIN
    NODE_BIN="$(command -v node 2>/dev/null)"
    if [ -z "$NODE_BIN" ]; then
        c_info "node tidak ditemukan di PATH — cron reconnect dilewati (pasang manual)"
        return 0
    fi
    # runner harus ada dulu
    if [ ! -f "${BACKEND_DIR}/jalan-sinkron-hotspot.js" ] || [ ! -f "${BACKEND_DIR}/jalan-deteksi-reboot.js" ]; then
        c_info "Runner reconnect belum ada di ${BACKEND_DIR} — cron dilewati"
        return 0
    fi
    ( crontab -l 2>/dev/null | grep -v "jalan-sinkron-hotspot\.js" | grep -v "jalan-deteksi-reboot\.js"
      echo "*/2 * * * * cd ${BACKEND_DIR} && ${NODE_BIN} jalan-sinkron-hotspot.js >/dev/null 2>&1"
      echo "*/2 * * * * cd ${BACKEND_DIR} && ${NODE_BIN} jalan-deteksi-reboot.js >/dev/null 2>&1"
    ) | crontab - 2>/dev/null
    local n_hs n_rb
    n_hs="$(crontab -l 2>/dev/null | grep -c 'jalan-sinkron-hotspot\.js')"
    n_rb="$(crontab -l 2>/dev/null | grep -c 'jalan-deteksi-reboot\.js')"
    if [ "$n_hs" = "1" ] && [ "$n_rb" = "1" ]; then
        c_ok "Cron reconnect terpasang (sinkron-hotspot + deteksi-reboot, tiap 2 menit)"
    else
        c_info "Cron reconnect: hotspot=${n_hs} reboot=${n_rb} (harusnya 1 & 1) — cek: crontab -l"
    fi
}

# Migrasi DB WAJIB jalan SEBELUM pm2 restart — kalau tidak, aplikasi menyala
# dengan kode yang butuh kolom permissions padahal kolomnya belum ada.
migrasi_permissions || c_err "Migrasi permissions gagal (RBAC bisa error)"
migrasi_durasi || c_err "Migrasi durasi gagal (fitur durasi hotspot bisa tak jalan)"
setup_noresetcounter || c_info "Setup noresetcounter dilewati (durasi hotspot perlu setup manual)"
migrasi_last_uptime || c_err "Migrasi last_uptime gagal (deteksi reboot bisa tak jalan)"
setup_cron_reconnect || c_info "Setup cron reconnect dilewati (non-fatal)"
