...
#   - git pull versi terbaru dari branch DIST (build terobfuscate) — JAGA .env,
#     uploads, node_modules
#   - npm install dependensi baru
#   - restart pm2
#
#  CATATAN PENTING:
#   - BRANCH="dist" => yang ditarik adalah build TEROBFUSCATE untuk pelanggan.
#   - Branch "master" = source plain (tempat developer kerja). JANGAN jalankan
#     script ini di VPS development/kerja — ia akan me-reset folder ke isi dist.
# =============================================================================
set -e
...
if [ -f "${BACKEND_DIR}/package.json" ]; then
    ...
    cd "$BACKEND_DIR"
    # Hindari unduh ulang Chrome bawaan Puppeteer tiap update (hemat ~170MB,
    # cegah "ENOSPC: no space left"). Hormati setelan .env; jika tak ada,
    # skip unduh bila Chromium sistem tersedia.
    PPTR_ENV=""
    if grep -q '^PUPPETEER_SKIP_DOWNLOAD=' "${BACKEND_DIR}/.env" 2>/dev/null; then
        PPTR_ENV="PUPPETEER_SKIP_DOWNLOAD=$(grep -m1 '^PUPPETEER_SKIP_DOWNLOAD=' "${BACKEND_DIR}/.env" | cut -d= -f2-)"
    elif command -v chromium-browser >/dev/null 2>&1 || command -v chromium >/dev/null 2>&1; then
        PPTR_ENV="PUPPETEER_SKIP_DOWNLOAD=true"
    fi
    env $PPTR_ENV npm install --no-audit --no-fund 2>&1 | tail -3 && c_ok "Dependensi siap" \
        || { c_err "npm install gagal"; exit 1; }
else
    c_err "package.json tidak ditemukan di ${BACKEND_DIR} — lewati npm install"
fi

# 4-heal) Pastikan skema DB lengkap (self-heal untuk instalasi lama yang kurang
#         kolom/role). Semua idempotent & aman dijalankan berulang.
ENVF="${BACKEND_DIR}/.env"
if [ -f "$ENVF" ]; then
    DBU=$(grep -m1 -E '^DB_USER=' "$ENVF" | cut -d= -f2-)
    DBP=$(grep -m1 -E '^DB_PASS=' "$ENVF" | cut -d= -f2-)
    DBN=$(grep -m1 -E '^DB_NAME=' "$ENVF" | cut -d= -f2-)
    if [ -n "$DBU" ] && [ -n "$DBN" ]; then
        mysql -u "$DBU" -p"$DBP" "$DBN" <<'SQL' 2>/dev/null || true
...
UPDATE admin SET role='superadmin' WHERE (role IS NULL OR role='') AND username='admin';
SQL
        c_ok "Skema DB diperiksa (kolom paket & role admin dipastikan)"
    fi
fi
