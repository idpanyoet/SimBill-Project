...
# 4c) Selaraskan versi di package.json (agar pm2/npm menampilkan versi yang benar).
#     Ambil bagian semver dari tag (mis. v1.8.1 -> 1.8.1). Kalau tak berpola semver
#     (mis. fallback 'git-<hash>'), LEWATI supaya package.json tetap valid.
#     sed hanya menyentuh baris "version" PERTAMA (bukan key "version" milik dependensi).
SEMVER=$(printf '%s' "$LATEST_TAG" | sed -nE 's/^v?([0-9]+\.[0-9]+\.[0-9]+).*/\1/p')
if [ -n "$SEMVER" ]; then
    for PJ in "${BACKEND_DIR}/package.json" "${APP_DIR}/package.json"; do
        [ -f "$PJ" ] || continue
        sed -i -E "0,/\"version\"[[:space:]]*:[[:space:]]*\"[^\"]*\"/ s//\"version\": \"${SEMVER}\"/" "$PJ"
    done
    c_ok "package.json diselaraskan ke ${SEMVER}"
fi
