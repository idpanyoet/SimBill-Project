...
# Durasi hotspot butuh FreeRADIUS noresetcounter (sqlcounter) untuk membatasi
# TOTAL uptime voucher (akumulasi). Idempotent: aman dijalankan tiap update.
#   - enable module sqlcounter (symlink mods-available -> mods-enabled)
#   - panggil 'noresetcounter' di authorize section (setelah -sql)
#   - freeradius -CX (cek config) SEBELUM reload; batal bila config rusak.
# Fitur durasi backward-compatible: kalau tak dipakai, tak berpengaruh.
setup_noresetcounter() {
    RADDIR="$(ls -d /etc/freeradius/*/ 2>/dev/null | head -1)"
    [ -z "$RADDIR" ] && { c_info "Durasi FreeRADIUS dilewati (FreeRADIUS tidak ada)"; return 0; }
    local MA="${RADDIR}mods-available/sqlcounter"
    local ME="${RADDIR}mods-enabled/sqlcounter"
    local SITE="${RADDIR}sites-enabled/default"
    [ -f "$MA" ] || { c_info "Durasi dilewati (mods-available/sqlcounter tak ada)"; return 0; }
    [ -f "$SITE" ] || { c_info "Durasi dilewati (sites-enabled/default tak ada)"; return 0; }

    local BERUBAH=0

    # 1) enable sqlcounter (symlink) bila belum
    if [ ! -e "$ME" ]; then
        ln -sf ../mods-available/sqlcounter "$ME" && BERUBAH=1
    fi

    # 2) panggil noresetcounter di authorize (setelah -sql) bila belum
    if ! grep -qE '^\s*noresetcounter' "$SITE"; then
        # sisip setelah baris '-sql' PERTAMA yang ada di dalam authorize.
        # cari nomor baris '-sql' pertama (indentasi apa pun), sisip setelahnya.
        local LN
        LN="$(grep -nE '^\s*-sql\s*$' "$SITE" | head -1 | cut -d: -f1)"
        if [ -n "$LN" ]; then
            cp "$SITE" "${SITE}.bak-durasi-$(date +%s)" 2>/dev/null
            # ambil indentasi baris -sql biar konsisten
            local IND
            IND="$(sed -n "${LN}p" "$SITE" | sed -E 's/([[:space:]]*).*/\1/')"
            sed -i "${LN}a\\${IND}noresetcounter" "$SITE" && BERUBAH=1
        else
            c_info "Durasi: baris '-sql' di authorize tak ditemukan — lewati (setup manual)"
        fi
    fi

    # 3) kalau ada perubahan → cek config lalu reload (aman)
    if [ "$BERUBAH" = "1" ]; then
        if command -v freeradius >/dev/null 2>&1; then
            if freeradius -CX >/dev/null 2>&1; then
                ...
                c_err "FreeRADIUS config error setelah setup durasi — DIBATALKAN."
                c_err "  Cek: freeradius -CX  (backup config ada di ${SITE}.bak-durasi-*)"
            fi
        else
            systemctl reload freeradius >/dev/null 2>&1 || true
            c_info "Durasi noresetcounter dipasang (freeradius -CX tak tersedia untuk verifikasi)"
        fi
    else
