...
setup_acslite() {
    ...
    # DB 'acs' + user khusus (login root via socket; TIDAK mengubah password root).
    local DBPASS APIKEY
...
SQL
    then
        c_info "ACS Lite dilewati (gagal siapkan DB — root MariaDB perlu password?)"; return 0
    fi

    umask 177
    cat > "${ACS_DIR}/.env" <<EOF
ACS_PORT=${ACS_PORT}
DB_DSN=acs_user:${DBPASS}@tcp(127.0.0.1:3306)/acs?parseTime=true
API_KEY=${APIKEY}
EOF
    umask 022
