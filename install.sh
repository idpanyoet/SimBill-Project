...
step "2c/7 VPN (WireGuard & L2TP/IPSec)"
# MASALAH LAMA (diperbaiki v1.13.4):
#   Installer dulu HANYA memasang binary xl2tpd, lalu systemd menjalankannya —
#   sementara KONFIGURASI baru ditulis kalau admin menekan tombol "Install L2TP"
#   di panel. Akibatnya xl2tpd hidup memakai /etc/xl2tpd/xl2tpd.conf BAWAAN UBUNTU
#   yang SELURUH ISINYA COMMENT (termasuk [lns default]) → xl2tpd MENOLAK SEMUA
#   koneksi: "Denied connection to unauthorized peer".
#   Panel pun menampilkan "L2TP aktif" (karena service running), jadi tidak ada
#   yang curiga. Kasus nyata: MikroTik pelanggan retry berjam-jam tanpa hasil.
#
#   Sekarang installer LANGSUNG menulis konfigurasi yang benar. Selesai install,
#   L2TP siap pakai — tanpa perlu menekan tombol apa pun.
INSTALL_VPN="${INSTALL_VPN:-y}"
L2TP_IP_LOCAL="${L2TP_IP_LOCAL:-10.10.29.1}"
L2TP_IP_RANGE="${L2TP_IP_RANGE:-10.10.29.10-10.10.29.100}"
L2TP_PSK="${L2TP_PSK:-}"        # kosong = digenerate acak
...
if [[ "${INSTALL_VPN,,}" == y* ]]; then
  ...
  ok "Tools VPN terpasang (${VPN_OK} paket)"

  if command -v xl2tpd >/dev/null 2>&1; then
    # ── xl2tpd.conf ──────────────────────────────────────────────────────────
    # Bawaan Ubuntu 100% comment → TIDAK ADA [lns default] → semua peer ditolak.
    # Ditulis ulang bersih. Backup dulu kalau ada isi yang bukan comment.
    mkdir -p /etc/xl2tpd
    if [ -f /etc/xl2tpd/xl2tpd.conf ] && grep -qE '^\s*\[lns default\]' /etc/xl2tpd/xl2tpd.conf; then
      info "xl2tpd.conf sudah punya [lns default] — TIDAK ditimpa"
    else
      [ -f /etc/xl2tpd/xl2tpd.conf ] && cp /etc/xl2tpd/xl2tpd.conf "/etc/xl2tpd/xl2tpd.conf.bak-$(date +%s)"
      cat > /etc/xl2tpd/xl2tpd.conf <<L2TPCONF
[global]
port = 1701

[lns default]
ip range = ${L2TP_IP_RANGE}
local ip = ${L2TP_IP_LOCAL}
require chap = yes
refuse pap = no
...
length bit = yes
L2TPCONF
      ok "xl2tpd.conf ditulis ([lns default] · ${L2TP_IP_RANGE})"
    fi

    # ── options.l2tpd.lns ────────────────────────────────────────────────────
    # Opsi modem (crtscts/lock/idle) SENGAJA tidak dipakai: tidak valid untuk
    # L2TP dan membuat pppd keluar (exit 2) tiap tunnel terbentuk.
    # mtu/mru 1400 penting di VPS ber-NAT.
    if [ ! -f /etc/ppp/options.l2tpd.lns ]; then
      mkdir -p /etc/ppp
      cat > /etc/ppp/options.l2tpd.lns <<PPPOPTS
ipcp-accept-local
ipcp-accept-remote
require-chap
refuse-pap
...
connect-delay 5000
PPPOPTS
      ok "options.l2tpd.lns dibuat"
    else
      info "options.l2tpd.lns sudah ada — tidak diubah"
    fi

    touch /etc/ppp/chap-secrets && chmod 600 /etc/ppp/chap-secrets

    # ── IPSec (opsional; L2TP murni tetap jalan tanpa ini) ───────────────────
...
IPSECCONF
    ...
    if [ ! -f /etc/ipsec.secrets ]; then
      [ -z "$L2TP_PSK" ] && L2TP_PSK="$(head -c 18 /dev/urandom | base64 | tr -d '/+=' | head -c 20)"
      printf ': PSK "%s"\n' "$L2TP_PSK" > /etc/ipsec.secrets
      chmod 600 /etc/ipsec.secrets
      ok "IPSec PSK dibuat (lihat: cat /etc/ipsec.secrets)"
    fi
    ...
    for svc in strongswan-starter strongswan ipsec; do
      systemctl list-unit-files "${svc}.service" 2>/dev/null | grep -q "${svc}.service" && { SWAN_SVC="$svc"; break; }
    done
    if [ -n "$SWAN_SVC" ]; then
      systemctl enable "$SWAN_SVC" >/dev/null 2>&1 || true
      systemctl restart "$SWAN_SVC" >/dev/null 2>&1 || true
    fi

    if grep -qE '^\s*\[lns default\]' /etc/xl2tpd/xl2tpd.conf && systemctl is-active --quiet xl2tpd; then
      ok "L2TP SIAP PAKAI — tambahkan user di panel: menu VPN → Tambah User L2TP"
    else
      info "xl2tpd terpasang tapi belum aktif — cek: systemctl status xl2tpd"
    fi
  fi
else
...
ENV
  ...
  # HARDENING #5: kunci CORS ke domain panel bila DOMAIN diisi (hindari fallback '*').
  # Panel disajikan same-origin, jadi ini defense-in-depth untuk origin lain.
  if [ -n "${DOMAIN:-}" ]; then
...
if [[ "${INSTALL_RADIUS,,}" == y* || "${INSTALL_VPN,,}" == y* ]]; then
  ...
  # PORT (diperbaiki v1.13.4): dulu HANYA 1812/1813/1701 yang dibuka, padahal
  # pesan di akhir install mengklaim "1701/500/4500". UDP 500 (IKE) dan 4500
  # (NAT-T) TIDAK PERNAH dibuka → setiap pelanggan yang memakai L2TP/IPSec
  # pasti gagal konek. ESP (protokol 50) juga perlu untuk IPSec non-NAT.
  if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -qi "Status: active"; then
    ...
    ok "Port RADIUS/L2TP/IPSec dibuka via ufw (UDP 1812/1813/1701/500/4500 + ESP)"
  else
    ...
    ok "Port RADIUS/L2TP/IPSec dibuka via iptables (UDP 1812/1813/1701/500/4500 + ESP + ppp+)"
  fi
