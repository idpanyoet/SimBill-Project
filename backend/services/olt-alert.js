...
async function pelangganFor(oltId, indexes) {
  ...
  const rows = await db.query(
    `SELECT l.onu_index, l.onu_name, l.sn, p.id AS pid, p.nama, p.username, p.status, p.tgl_expired
...
async function pollOlt(oltId) {
  ...
  // ── SEDIKIT: cek cause tiap ONU, cuma alert kalau DyingGasp/LOS ──
  for (const index of newlyOffline) {
    ...
    if (!CAUSE_RE.test(cause)) continue; // bukan gangguan beneran (mis reboot) -> skip
