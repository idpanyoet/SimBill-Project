...
async function tick() {
  try {
    ...
    for (const o of olts) {
      // OLT EPON (VSOL/HIOSO) via SNMP: tak ada data online/offline & cause (DyingGasp/LOS),
      // dan getOnuDetail akan SSH → gagal. Lagipula jumlah ONU fluktuatif tiap poll →
      // "offline" palsu. Jadi alert gangguan ONU hanya untuk OLT ZTE (GPON via SSH).
      if (o.vendor && o.vendor !== 'zte') continue;
