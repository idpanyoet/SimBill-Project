...
async function listDevices({ limit = 1000 } = {}) {
    ...
    const projection = [
        ...
        // VLAN inet — path beda per vendor
        ...
        'InternetGatewayDevice.WANDevice.2.WANConnectionDevice.2.WANIPConnection.1.X_ZTE-COM_VLANID',
...
function normalizeDevice(d) {
    ...
    const ambilKoneksi = (parent, type) => {
        ...
        for (const j of Object.keys(all)) {
            ...
            // VLAN inet — path beda per vendor (ZTE/Huawei/umum)
            const vlan = gv(c, 'X_ZTE-COM_VLANID') ?? gv(c, 'X_HW_VLAN') ??
                         gv(c, 'X_CT-COM_VLANIDMark') ?? gv(c, 'VLANIDMark') ?? null;
            if (ip && ip !== '0.0.0.0') {
                // koneksi aktif (punya IP WAN) → sumber paling andal
                wanIp = ip;
                if (user) pppUsername = user;
                if (vlan !== null && vlan !== '' && String(vlan) !== '0') vlanInet = vlan;
            } else {
                if (!wanIp && ip) wanIp = ip;                 // simpan 0.0.0.0 sbg fallback
                if (user && !pppUsername) pppUsername = user; // username fallback
                if (vlanInet === '' && vlan !== null && String(vlan) !== '0') vlanInet = vlan;
            }
        }
    };
