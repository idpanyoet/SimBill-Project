...
async function verifikasiSignatureMidtrans(body) {
    ...
    // FAIL-CLOSED: tanpa server key (mis. provider aktif bukan Midtrans), JANGAN
    // loloskan. Kalau serverKey='', hash = SHA512(order_id+status_code+gross_amount)
    // yang seluruh isinya dari body → siapa pun bisa memalsukan callback "settlement".
    if (!cfg.serverKey) {
        console.warn('[PAYMENT] Midtrans server key kosong — callback ditolak');
        return false;
    }
    const hash = crypto
        .createHash('sha512')
    ...
    const a = Buffer.from(String(signature_key || ''), 'utf8');
