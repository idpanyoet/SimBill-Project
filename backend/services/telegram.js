...
async function notifPendaftaran(data) {
    try {
        ...
        const { nama, alamat, paket, latitude, longitude, no_hp, username, ktp_url, jarak_kabel } = data;
        ...
        let teks = `🆕 <b>Pendaftaran Pelanggan Baru</b>\n\n`
            ...
            + `📍 Alamat: ${alamat || '-'}\n`
            + (no_hp ? `📞 ${no_hp}\n` : '')
            + ((jarak_kabel !== undefined && jarak_kabel !== null && jarak_kabel !== '') ? `📏 Jarak kabel (ODP/OLT): ${jarak_kabel} m\n` : '')
...
// ── ACS Lite (GoACS): ambil device + normalisasi ke bentuk seperti GenieACS ──
...
async function _acsliteCfg() {
    ...
    return { url: (m.acslite_url || 'http://127.0.0.1:7547').replace(/\/+$/, ''), key: _acsliteKey() || (m.acslite_api_key || '') };
}
...
function _acsIP(d) {
    ...
    if (d.connection_request_url) { const m = String(d.connection_request_url).match(/https?:\/\/([0-9.]+)/); if (m) return m[1]; }
    return '';
}
// Filter device "probe"/dummy GoACS (serial semua-nol atau manufacturer/product 'probe').
function _acsIsProbe(d) {
    const sn = String(d.serial_number || '');
    const mf = String(d.manufacturer || '').toLowerCase();
    const pc = String(d.product_class || '').toLowerCase();
    return /^0+$/.test(sn) || mf === 'probe' || pc === 'probe';
}
async function listDevicesAcslite() {
    const cfg = await _acsliteCfg();
    let raw = [];
    try {
        const r = await axios.get(`${cfg.url}/api/devices`, {
            headers: cfg.key ? { 'X-API-Key': cfg.key } : {}, params: { per_page: 2000 }, timeout: 6000
        });
        raw = Array.isArray(r.data) ? r.data : (r.data && r.data.data ? r.data.data : []);
    } catch (e) { return []; }
    return raw.filter(d => d && d.serial_number && !_acsIsProbe(d)).map(d => {
        const li = d.last_inform_time || null;
        let online = false;
        if (li) { const mins = (Date.now() - new Date(li).getTime()) / 60000; online = mins >= -5 && mins <= 30; }
        return {
            serial_number: d.serial_number, manufacturer: d.manufacturer,
            product_class: d.product_class || '', model: d.product_class || '',
            pppoe_username: _acsPPPoE(d), ip_address: _acsIP(d),
            rx_power: (d.rx_power !== undefined ? d.rx_power : null),
            last_inform: li, status: online ? 'online' : 'offline',
            genie_id: null, _source: 'acslite'
        };
    });
}
// Gabungan GenieACS + ACS Lite. Dedup by serial → pilih yang PALING BARU inform
// (ONU yang dipindah ke ACS Lite akan basi di GenieACS). Probe GoACS otomatis dibuang.
async function listDevicesGabungan(opts) {
    const [g, a] = await Promise.all([
        genie.listDevices(opts).catch(() => []),
        listDevicesAcslite().catch(() => []),
    ]);
    const ts = x => { const t = x && x.last_inform; const v = t ? new Date(t).getTime() : 0; return isNaN(v) ? 0 : v; };
    const bySerial = {};
    [...(g || []), ...(a || [])].forEach(d => {
        const sn = String(d.serial_number || '').toLowerCase();
        if (!sn) return;
        if (!bySerial[sn] || ts(d) >= ts(bySerial[sn])) bySerial[sn] = d;
    });
    return Object.values(bySerial);
}
// Refresh device sesuai sumber ACS-nya.
async function refreshDeviceGabungan(dev) {
    try {
        if (dev._source === 'acslite') {
            const cfg = await _acsliteCfg();
            await axios.post(`${cfg.url}/api/refresh`, null, {
                headers: cfg.key ? { 'X-API-Key': cfg.key } : {}, params: { sn: dev.serial_number }, timeout: 6000
            });
        } else if (dev.genie_id) {
            await genie.refreshDevice(dev.genie_id);
        }
    } catch (e) {}
}
...
async function cekRedaman(identifier) {
    ...
    // Sumber data = LIVE dari GenieACS + ACS Lite (gabungan), cocokkan ke
    // serial ATAU username PPPoE. Device dipilih dari ACS yang paling baru inform.
    let list = [];
    try {
        list = await listDevicesGabungan({ limit: 2000 });
    } catch (e) {
        return `⚠️ Gagal hubungi ACS: ${e.message}`;
    }
    ...
    try { await refreshDeviceGabungan(match); } catch (e) {}
...
async function cekPelanggan(nama) {
    ...
    // 3) Data ONU live dari GenieACS + ACS Lite (match by PPPoE username = p.username)
    let dev = null;
    try {
        const list = await listDevicesGabungan({ limit: 5000 });
