...
async function _kirimPesanLangsung(no_hp, pesan, pelanggan_id = null, tipe = 'manual', invoice_id = null) {
    ...
    try {
        ...
        // Deteksi kegagalan "soft": provider balas HTTP 200 tapi menolak
        // (mis. Fonnte {"status":false,"reason":"invalid token"}). Tanpa ini,
        // semua kegagalan tercatat 'sent' dan tak terlihat.
        const _rd = response && response.data;
        if (_rd && _rd.status === false) {
            const _reason = _rd.reason || _rd.message || JSON.stringify(_rd);
            await query(`UPDATE wa_log SET status='failed', response=? WHERE id=?`, [JSON.stringify(_rd), logId]);
            console.error(`[WA] Ditolak ${cfg.provider} ke ${no_hp}: ${_reason}`);
            return { sukses: false, error: _reason };
        }
...
async function kirimKonfirmasiBayar(opt) {
    ...
    // member_id / uid pelanggan (via invoice) untuk placeholder [member_id] / [uid]
    let v_member_id = (opt && opt.member_id != null) ? opt.member_id : null;
    let v_uid = (opt && (opt.uid || opt.username)) || null;
    if ((v_member_id == null || !v_uid) && v_no_invoice) {
        try {
            const _p = await queryOne(
                `SELECT p.id, p.username FROM invoice i JOIN pelanggan p ON p.id=i.pelanggan_id
                 WHERE i.no_invoice=? LIMIT 1`, [v_no_invoice]);
            if (_p) { if (v_member_id == null) v_member_id = _p.id; if (!v_uid) v_uid = _p.username; }
        } catch (_e) { /* biarkan '-' */ }
    }
    ...
    const data = {
        // identitas
        nama: nama, fullname: nama,
        member_id: (v_member_id != null ? String(v_member_id) : '-'),
        uid: v_uid || '-', username: v_uid || '-',
        // nominal
...
async function kirimPelangganBaru(opt) {
    ...
    const {
        ...
        prorata, hari_prorata, no_invoice, payment_url, aktif, lunas
    } = opt || {};
    ...
    // aktif: true = sudah aktif (paid), false = belum aktif (unpaid, harus bayar dulu).
    // Default true bila tak dikirim (registrasi biasa dianggap aktif).
    const _aktif = (aktif === undefined || aktif === null) ? true : !!aktif;
    const _adaUrl = payment_url && payment_url !== '-';
    // {blok_pembayaran} = blok link bayar yang HANYA muncul untuk pelanggan BELUM BAYAR
    // (prepaid + Fixed Date + unpaid). Kalau sudah aktif/tak ada link → string kosong.
    const _blokPembayaran = (!_aktif && _adaUrl)
        ? `💳 *Pembayaran* (bayar untuk aktivasi):\n${payment_url}`
        : '';
    ...
    const data = {
        ...
        billing_status: _aktif ? 'Aktif' : 'Belum Aktif',
        status_aktif: _aktif ? 'AKTIF' : 'BELUM AKTIF — menunggu pembayaran',
        ...
        // Prorata bulan pertama (prepaid + Fixed Date)
        prorata: prorata ? 'Ya' : 'Tidak',
        hari_prorata: (hari_prorata != null ? String(hari_prorata) : '-'),
        no_invoice: no_invoice || '-', invoice: no_invoice || '-',
        payment_url: payment_url || '-', link_bayar: payment_url || '-',
        // {total_bersih} = nominal TANPA prefix "Rp" (biar template bebas kasih "Rp" sendiri)
        total_bersih: angka(totalFinal),
        // {tagihan_pertama} = teks rincian yg auto-nyesuaikan (prorata/penuh) + status lunas
        tagihan_pertama: (prorata
            ? `Rp ${angka(totalFinal)} (prorata ${hari_prorata} hari)`
            : `Rp ${angka(totalFinal)}`) + (lunas ? ' — LUNAS ✅' : ''),
        // {status_bayar} = "LUNAS" / "BELUM DIBAYAR"
        status_bayar: lunas ? 'LUNAS ✅' : 'BELUM DIBAYAR',
        // {blok_pembayaran} = blok link bayar, HANYA untuk pelanggan belum bayar (unpaid)
        blok_pembayaran: _blokPembayaran
    };
    const pesan = _isiTemplate(tpl, data);
...
module.exports = {
    kirimPesanLangsung: _kirimPesanLangsung,
    kirimPesan,
    kirimDokumen,
    kirimReminder,
    kirimLinkBayar,
