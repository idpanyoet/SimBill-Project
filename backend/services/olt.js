...
let eponSnmp = null;
try { eponSnmp = require('./olt-epon-snmp'); } catch (e) { eponSnmp = null; }
...
function loadOlts() {
  try {
    ...
    return list.map((o, i) => ({
      ...
      password: o.password,
      model: o.model || 'C300',
      vendor: (o.vendor || 'zte').toLowerCase(),
      type: (o.type || 'gpon').toLowerCase(),
      snmp: o.snmp || null,
    }));
  } catch (e) {
...
function listOlts() {
  ...
  return loadOlts().map(({ id, name, host, port, model, vendor, type }) => ({ id, name, host, port, model, vendor, type }));
}

// Vendor EPON (non-ZTE) yang dilayani via SNMP driver
function isEponVendor(olt) {
  return olt && eponSnmp && olt.vendor && olt.vendor !== 'zte' && !!eponSnmp.profileFor(olt);
}

// Startup: prewarm cache untuk OLT EPON agent-lambat (mis. HIOSO) agar panel
// selalu dapat data instan (walk berat ~2 menit berjalan di belakang layar).
if (eponSnmp && typeof eponSnmp.prewarm === 'function') {
  setTimeout(() => {
    try {
      for (const o of loadOlts()) {
        if (isEponVendor(o)) eponSnmp.prewarm(o);   // hanya profil slowAgent yang benar-benar jalan
      }
    } catch (e) { console.warn('[olt] prewarm gagal:', e.message); }
  }, 5000);
}
...
async function setLink(oltId, index, pelangganId, extra = {}) {
  ...
  cache.delete(`dash:${olt.id}`); cache.delete(`state:${olt.id}`);   // paksa refresh grid
  return { ok: true };
}
...
function _buildDashboard(olt) {
  if (isEponVendor(olt)) {
    return (async () => {
      const state = await eponSnmp.getOnuState(olt);   // { onus, online, offline, total }
      ...
      const onus = state.onus.map((o) => {
        ...
        return { ...o, sn: o.sn || (lk && lk.sn) || null, type: null,
          pelanggan: lk ? lk.pelanggan : null, tag: lk ? lk.tag : null };
      });
      const data = {
        olt: { id: olt.id, name: olt.name, model: olt.model, vendor: olt.vendor, type: olt.type },
        cards: [],
...
async function getOnuInfo(oltId, index) {
  const _olt = getOlt(oltId);
  if (isEponVendor(_olt)) {
    const [st, linkMap] = await Promise.all([eponSnmp.getOnuState(_olt), getLinkMap(oltId)]);
    const onu = (st.onus || []).find((o) => o.index === index) || {};
    return {
      index,
      detail: { mac: onu.sn || null, port: onu.port, onuId: onu.onuId,
        state: onu.online ? 'online' : 'offline',
        note: 'OLT EPON (VSOL) — monitoring via SNMP. Detail optik & aksi CLI belum tersedia.' },
      power: null,
      pelanggan: linkMap[index] ? linkMap[index].pelanggan : null,
      tag: linkMap[index] ? linkMap[index].tag : null,
    };
  }
...
async function getOnuState(oltId) {
  ...
  if (!olt) throw new Error('OLT tidak ditemukan');
  if (isEponVendor(olt)) {
    return cached(`state:${olt.id}`, 20000, () => eponSnmp.getOnuState(olt));
  }
...
function validIndex(idx) {
  // ZTE GPON: rack/slot/port:onu (1/8/1:1)  |  EPON: port:onuId (1:1)
  return /^\d+\/\d+\/\d+:\d+$/.test(idx) || /^\d+:\d+$/.test(idx);
}

async function getOnuPower(oltId, index) {
  const _olt = getOlt(oltId);
  if (isEponVendor(_olt)) return { rx: null, tx: null, note: 'RX power tidak tersedia via SNMP (EPON VSOL)' };
  if (!validIndex(index)) throw new Error('Index ONU tidak valid (format: 1/8/1:1)');
  const olt = getOlt(oltId);
  if (!olt) throw new Error('OLT tidak ditemukan');
...
function listOltsForEdit() {
  return loadOltsRaw().map((o) => ({
    ...
    vendor: (o.vendor || 'zte').toLowerCase(), type: (o.type || 'gpon').toLowerCase(),
...
function upsertOlt(cfg = {}) {
  ...
  const vendor = String(cfg.vendor || 'zte').trim().toLowerCase();
  const type = String(cfg.type || 'gpon').trim().toLowerCase();
  // EPON (VSOL/HSGQ/HIOSO) = monitoring SNMP; username/password OLT opsional.
  const isEpon = vendor !== 'zte' || type === 'epon';

  let username = String(cfg.username || '').trim();
  if (!username) { if (isEpon) username = '-'; else throw new Error('Username kosong'); }
  ...
  if (!password) { if (isEpon) password = '-'; else throw new Error('Password kosong'); }
  ...
  const entry = { id, name, model, vendor, type, host, port, username, password };
  ...
  return { id, name, host, port, model, vendor, type, hasSnmp: !!snmp, updated: idx >= 0 };
}
