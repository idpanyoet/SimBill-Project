...
/**
 * Fixed Date berikutnya setelah tglAktif (fixedDay = tanggal tetap, mis. 1).
 * Clamp 1..28 agar aman di semua bulan.
 */
function nextFixedDate(tglAktif, fixedDay) {
    const dayjs = require('dayjs');
    const a = dayjs(tglAktif);
    const d = Math.min(Math.max(parseInt(fixedDay) || 1, 1), 28);
    let x = a.date(d);
    if (!x.isAfter(a)) x = x.add(1, 'month');
    return x;
}

/**
 * Prorata invoice pertama (PREPAID + Fixed Date/kalender, paket bulanan).
 *   harga, tglAktif(YYYY-MM-DD), fixedDay(dari setting billing_tgl_invoice),
 *   basis 'flat30'|'actual', minHari (ambang "mepet" → geser ke Fixed Date berikutnya).
 * return { jumlah, tglExpired, hariTerpakai, hariSebulan, rasio, digeser }
 */
function hitungProrata(harga, tglAktif, fixedDay, basis = 'flat30', minHari = 7) {
    const dayjs = require('dayjs');
    const a = dayjs(tglAktif);
    let expired = nextFixedDate(tglAktif, fixedDay);
    let hariTerpakai = expired.diff(a, 'day');
    let digeser = false;
    if (hariTerpakai < minHari) { expired = expired.add(1, 'month'); hariTerpakai = expired.diff(a, 'day'); digeser = true; }
    let hariSebulan;
    if (basis === 'actual') { hariSebulan = expired.diff(expired.subtract(1, 'month'), 'day'); }
    else { hariSebulan = 30; }
    const rasio = Math.max(0, Math.min(1, hariTerpakai / hariSebulan));
    return { jumlah: Math.round(Number(harga) * rasio), tglExpired: expired.format('YYYY-MM-DD'), hariTerpakai, hariSebulan, rasio: +rasio.toFixed(4), digeser };
}

module.exports = { pool, query, queryOne, withTransaction, generateUniqueInvoiceNo, hitungExpired, hitungExpiredDari, nextFixedDate, hitungProrata };
