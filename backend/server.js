...
// Middleware maintenance: saat aktif, portal PELANGGAN diblokir (admin panel tetap normal).
// Nilai di-cache 15 detik agar tidak query DB tiap request.
let _maintCache = { val: false, at: 0 };
app.use('/api/client', async (req, res, next) => {
    try {
        if (Date.now() - _maintCache.at > 15000) {
            const rows = await require('./config/db').query("SELECT nilai FROM setting WHERE kunci='maintenance_mode' LIMIT 1");
            const r = Array.isArray(rows) ? rows[0] : rows;
            _maintCache = { val: !!(r && r.nilai === '1'), at: Date.now() };
        }
        if (_maintCache.val) {
            return res.status(503).json({ maintenance: true, error: 'Sistem sedang dalam pemeliharaan. Silakan coba beberapa saat lagi.' });
        }
    } catch (e) {}
    next();
});
...
// ── Deteksi IP publik server (untuk panduan isolir MikroTik). Cache 1 jam. ──
let _srvIP = null, _srvIPAt = 0;
app.get('/api/server-ip', (req, res) => {
    const now = Date.now();
    if (_srvIP && (now - _srvIPAt) < 3600000) return res.json({ ip: _srvIP, cached: true });
    const isIP = s => /^\d{1,3}(\.\d{1,3}){3}$/.test(String(s || '').trim());
    const hostFallback = () => {
        const h = (req.headers.host || '').split(':')[0];
        if (isIP(h)) return h;
        try {
            const os = require('os'), ifs = os.networkInterfaces();
            for (const n in ifs) for (const i of ifs[n]) if (i.family === 'IPv4' && !i.internal) return i.address;
        } catch (e) {}
        return null;
    };
    let done = false;
    const finish = (ip, source) => {
        if (done) return; done = true;
        if (isIP(ip)) { _srvIP = String(ip).trim(); _srvIPAt = now; return res.json({ ip: _srvIP, source }); }
        const fb = hostFallback();
        res.json({ ip: fb, source: fb ? 'fallback' : null });
    };
    try {
        const r = require('https').get('https://api.ipify.org', { timeout: 4000 }, resp => {
            let data = ''; resp.on('data', c => data += c); resp.on('end', () => finish(data, 'ipify'));
        });
...
// Speed-bump anti-flood: signature/secret sudah jadi gerbang auth, jadi limiter ini
// murni redam DoS. Ceiling sengaja TINGGI (600/menit/IP) supaya tak pernah menyentuh
// burst callback gateway/Telegram yang sah (mereka datang dari sedikit IP tetap),
// tapi tetap menahan banjir request dari satu IP. Turunkan bila perlu lebih ketat.
...
app.use('/webhook/wa', require('./routes/wa-command'));   // bot perintah WhatsApp (publik, secret token)
