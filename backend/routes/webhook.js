...
router.post('/duitku', async (req, res) => {
    try {
        ...
        // FAIL-CLOSED: tanpa API key (mis. provider aktif bukan Duitku), JANGAN
        // loloskan. Kalau apiKey='', signature = MD5(merchantCode+amount+merchantOrderId)
        // yang seluruh isinya dari body → siapa pun bisa memalsukan callback resultCode='00'
        // untuk melunasi invoice mana pun / menambah saldo reseller secara gratis.
        if (!cfg.apiKey) {
            console.warn('[WEBHOOK] Duitku API key kosong — callback ditolak');
            return res.status(401).send('API key belum dikonfigurasi');
        }
        // Pastikan merchantCode di body cocok dengan yang dikonfigurasi (tolak merchant asing)
        if (cfg.merchantCode && String(merchantCode) !== String(cfg.merchantCode)) {
            console.warn('[WEBHOOK] Duitku merchantCode tidak cocok — ditolak');
            return res.status(401).send('Merchant tidak dikenal');
        }
        ...
        // Bandingkan konstan-waktu (hindari timing attack)
...
// ============================================================
// Ambil nominal yang benar-benar dibayar dari body callback. Tiap gateway pakai
// field berbeda: Midtrans=gross_amount, Duitku/Xendit/Tripay=amount,
// Xendit jg paid_amount, Tripay jg total_amount (amount+fee). Return number,
// atau null bila tak bisa ditentukan (kalau null → JANGAN tolak, hindari
// menolak pembayaran sah hanya karena format field tak dikenal).
function _amountBayar(raw) {
    if (!raw || typeof raw !== 'object') return null;
    const kandidat = [raw.gross_amount, raw.amount, raw.paid_amount, raw.total_amount];
...
async function _prosesKonfirmasiBayar(order_id, payment_type, sukses, cancelled, rawBody) {
    ...
    if (sukses && order_id.startsWith('TOP-')) {
        const handled = await prosesTopupWebhook(order_id, payment_type, _amountBayar(rawBody));
    ...
    // GUARD NOMINAL (defense-in-depth). Untuk Midtrans/Duitku/Tripay nominal sudah
    // terikat di signature; tapi Xendit hanya cocokkan token (amount TIDAK terikat)
    // → tanpa cek ini, callback sah-token dgn nominal dikurangi bisa melunasi tagihan
    // penuh. Tolak bila nominal yang dibayar KURANG dari tagihan. Nominal sama/lebih
    // diloloskan (gateway selalu menagih persis, jadi legit selalu sama).
    if (sukses) {
        const tagihan = Math.round(Number(inv?.jumlah ?? invVoucher?.jumlah) || 0);
        const dibayar = _amountBayar(rawBody);
        if (tagihan > 0 && dibayar !== null && Math.round(dibayar) < tagihan) {
            console.warn(`[WEBHOOK] ⚠️ Nominal bayar (${Math.round(dibayar)}) < tagihan (${tagihan}) untuk ${order_id} — TOLAK, tidak ditandai lunas.`);
            return;
        }
    }
    ...
    if (invVoucher && sukses) {
        ...
        // ── Anti-dobel voucher (callback gateway bisa masuk 2x / di-"Resend") ──
        // Klaim ATOMIK: satu UPDATE ber-kondisi (row-level lock). Hanya SATU callback
        // yang berhasil menandai 'VoucherProses'; callback lain dapat affectedRows=0 → skip.
        // Ini mencegah race "cek-lalu-tulis" yang bikin voucher dobel.
        const ketSekarang = invVoucher.keterangan || '';
        if (/VoucherDibuat:/.test(ketSekarang)) {
            console.log(`[WEBHOOK] Voucher sudah dibuat untuk ${order_id}, skip.`);
            return;
        }
        let claim;
        try {
            claim = await query(
                `UPDATE invoice SET keterangan = CONCAT(COALESCE(keterangan,''), ' — VoucherProses')
                 WHERE id=? AND keterangan NOT LIKE '%VoucherDibuat:%' AND keterangan NOT LIKE '%VoucherProses%'`,
                [invVoucher.id]);
        } catch (e) { claim = null; }
        if (!claim || !claim.affectedRows) {
            console.log(`[WEBHOOK] Voucher ${order_id} sudah/sedang diproses callback lain — skip (anti-dobel).`);
            return;
        }
        // Lepas klaim bila gagal, agar callback berikutnya bisa mencoba lagi.
        const lepasKlaim = async () => {
            try { await query(`UPDATE invoice SET keterangan = REPLACE(keterangan, ' — VoucherProses', '') WHERE id=?`, [invVoucher.id]); } catch (e) {}
        };
        ...
        try {
            ...
            // Ganti penanda 'VoucherProses' → 'VoucherDibuat: <username>'
            await query(`UPDATE invoice SET keterangan = CONCAT(REPLACE(COALESCE(keterangan,''), ' — VoucherProses', ''), ' — VoucherDibuat: ', ?) WHERE id=?`,
    ...
    if (sukses) {
        // KLAIM ATOMIK: hanya SATU proses yang berhasil transisi unpaid→paid.
        // Gateway sering retry callback (bahkan bisa barengan dgn klik "Sudah Bayar"
        // admin). Pola cek-lalu-tulis (inv.status !== 'paid' dari SELECT basi) rawan
        // race → WA/log dobel & masa aktif ke-extend 2x. UPDATE ber-kondisi + cek
        // affectedRows menjamin exactly-once.
        const claim = await query(
            `UPDATE invoice SET status='paid', tgl_bayar=NOW(), metode_bayar=?
             WHERE id=? AND status<>'paid'`,
            [payment_type, inv.id]
        );
        if (!claim.affectedRows) {
            console.log(`[WEBHOOK] Invoice ${order_id} sudah paid — skip dobel.`);
            return;
        }

        // PRORATA: invoice pertama (prepaid + Fixed Date) TIDAK boleh di-extend
        // sesiklus penuh. Patok tgl_expired ke tgl_jatuh_tempo (Fixed Date). Samakan
        // dgn logika bayar-tunai (invoice.js). Tanpa ini, prorata yang dibayar via
        // gateway malah maju sebulan dari expired lama → tanggal salah.
        const _isProrata = String(inv.keterangan || '').toLowerCase().startsWith('prorata');
        const tgl_expired = _isProrata
            ? (inv.tgl_jatuh_tempo ? dayjs(inv.tgl_jatuh_tempo).format('YYYY-MM-DD HH:mm:ss') : inv.tgl_expired_lama)
            : ((Number(inv.masa_aktif) > 0)
