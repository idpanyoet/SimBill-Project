...
async function migrateAdminTable() {
    ...
    // Kolom hak akses granular (JSON array string)
    await query(`ALTER TABLE admin ADD COLUMN IF NOT EXISTS permissions TEXT NULL`).catch(()=>{});
...
// Serialisasi daftar izin → JSON string (maks 300 key, hanya string)
function permStr(p) {
    if (!Array.isArray(p)) return null;
    return JSON.stringify(p.filter(x => typeof x === 'string').slice(0, 300));
}
...
router.get('/', async (req, res, next) => {
    try {
        ...
        rows.forEach(r => { try { r.permissions = r.permissions ? JSON.parse(r.permissions) : []; } catch(e){ r.permissions = []; } });
...
router.post('/', async (req, res, next) => {
    try {
        const { username, nama, email, password, no_hp, role = 'operator', permissions } = req.body;
        ...
        const result = await query(
            `INSERT INTO admin (username, nama, email, password, no_hp, role, permissions) VALUES (?,?,?,?,?,?,?)`,
            [username, nama || username, email || null, hash, no_hp || null, role, permStr(permissions)]
        );
...
router.put('/:id', async (req, res, next) => {
    try {
        const { username, nama, email, no_hp, role, aktif, password, permissions } = req.body;
        ...
        const permVal = (permissions !== undefined) ? permStr(permissions) : p.permissions;
        await query(`
            UPDATE admin SET username=?, nama=?, email=?, no_hp=?, role=?, aktif=?, password=?, permissions=?, updated_at=NOW()
            WHERE id=?
        `, [username||p.username, nama||p.nama, email||p.email, no_hp||null, role||p.role, aktif!==undefined?aktif:p.aktif, hash, permVal, req.params.id]);
        tulisLog({ kategori:'System', pelaku: req.admin?.nama||'Admin',
