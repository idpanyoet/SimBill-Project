...
const KEYS = [
    ...
    'tg_ev_tiket', 'tg_ev_pelanggan', 'tg_ev_pembayaran', 'tg_ev_expired', 'tg_ev_suspend', 'tg_ev_pendaftaran', 'tg_ev_redaman',
    'tg_keterangan'
];
...
router.post('/webhook/aktifkan', async (req, res, next) => {
    try {
        const cfg = await tg.getCfg();
        if (!cfg.tg_bot_token) return res.status(400).json({ error: 'Bot token belum diisi & disimpan' });
...
router.post('/webhook/nonaktif', async (req, res, next) => {
    try {
