...
router.get('/devices', async (req, res) => {
    try {
        ...
        const out = [];
        for (const d of list) {
            // Lewati device probe/dummy GoACS (serial semua-nol / manufacturer/product 'probe')
            if (/^0+$/.test(String(d.serial_number || '')) ||
                String(d.manufacturer || '').toLowerCase() === 'probe' ||
                String(d.product_class || '').toLowerCase() === 'probe') continue;
