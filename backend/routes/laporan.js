...
router.get('/pengeluaran', async (req, res, next) => {
    try {
        ...
        const rows = await query(
            `SELECT id, tanggal, kategori, deskripsi, jumlah, metode_bayar, dibuat_oleh, created_at
...
router.post('/pengeluaran', requireAdmin, async (req, res, next) => {
    try {
        ...
        const a = req.admin || {};
        const r = await query(
            `INSERT INTO pengeluaran (tanggal, kategori, deskripsi, jumlah, metode_bayar, dibuat_oleh, dibuat_oleh_id)
             VALUES (?,?,?,?,?,?,?)`,
            [tanggal, (b.kategori || 'Lainnya').slice(0, 60), (b.deskripsi || '').slice(0, 255) || null,
             jumlah, (b.metode_bayar || '').slice(0, 40) || null, a.nama || a.username || null, a.id ?? null]);
...
router.put('/pengeluaran/:id', requireAdmin, async (req, res, next) => {
    try {
        ...
        if (!(jumlah >= 0)) return res.status(400).json({ error: 'Jumlah tidak valid' });
        await query(
            `UPDATE pengeluaran SET tanggal=?, kategori=?, deskripsi=?, jumlah=?, metode_bayar=? WHERE id=?`,
            [tanggal, (b.kategori || 'Lainnya').slice(0, 60), (b.deskripsi || '').slice(0, 255) || null,
             jumlah, (b.metode_bayar || '').slice(0, 40) || null, id]);
...
router.get('/pengeluaran/export', async (req, res, next) => {
    try {
        ...
        const { dari, sampai } = _rentang(req.query);
        const rows = await query(
            `SELECT tanggal, kategori, deskripsi, jumlah, metode_bayar, dibuat_oleh
        ...
        if ((req.query.format || 'csv') === 'excel') {
            ...
            rows.forEach(r => ws.addRow({ t: fmtTgl(r.tanggal), k: sf(r.kategori), d: sf(r.deskripsi), j: Number(r.jumlah), m: sf(r.metode_bayar), o: sf(r.dibuat_oleh) }));
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        ...
        let csv = 'Tanggal,Kategori,Deskripsi,Jumlah,Metode,Dibuat Oleh\n';
        rows.forEach(r => { csv += [fmtTgl(r.tanggal), esc(r.kategori), esc(r.deskripsi), Number(r.jumlah), esc(r.metode_bayar), esc(r.dibuat_oleh)].join(',') + '\n'; });
