...
router.get('/dashboard', async (req, res, next) => {
    try {
        ...
        const [pendapatanHari] = await query(`
            SELECT
                COALESCE(SUM(CASE WHEN DATE(tgl_bayar) = CURDATE() AND status = 'paid' THEN jumlah ELSE 0 END), 0) AS invoice_hari,
                COUNT(CASE WHEN DATE(tgl_bayar) = CURDATE() AND status = 'paid' THEN 1 END) AS invoice_count
            FROM invoice
        `);
        ...
        res.json({
            ...
            tagihan_bulan_ini:        tagihanBulanIni.total || 0,
            belum_bayar_count:        belumBayar.total,
            belum_bayar_nominal:      belumBayar.nominal || 0,
            vcr_pendapatan:           vcrPendapatan.total  || 0,
            vcr_jumlah:               vcrPendapatan.jumlah || 0,
            pendapatan_hari:          (pendapatanHari.invoice_hari || 0) + (vcrHari.total || 0),
            pendapatan_hari_trx:      (pendapatanHari.invoice_count || 0) + (vcrHari.jumlah || 0),
            wa_stats:                 waStats,
...
router.get('/export-excel', async (req, res, next) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Laporan_NetBilling_${tglStr}.xlsx"`);
