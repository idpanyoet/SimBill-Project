...
// ── GET /api/client/login-mode ────────────────────────────────
// Portal baca metode login aktif: 'otp' (default) atau 'sandi'
router.get('/login-mode', async (req, res) => {
    try {
        const r = await queryOne("SELECT nilai FROM setting WHERE kunci='client_login_mode' LIMIT 1").catch(() => null);
        const mode = (r && (r.nilai === 'sandi' || r.nilai === 'otp')) ? r.nilai : 'otp';
        res.json({ mode });
    } catch (e) { res.json({ mode: 'otp' }); }
});

// ── POST /api/client/login-sandi ──────────────────────────────
// Login pakai ID Pelanggan + sandi. Sandi default = nomor WhatsApp pelanggan.
// (Bila pelanggan sudah set sandi sendiri via portal, sandi bcrypt itu juga diterima.)
router.post('/login-sandi', async (req, res, next) => {
    try {
        const idPel = parseInt(req.body.id_pelanggan || req.body.id, 10);
        const sandi = String(req.body.sandi || req.body.password || '');
        if (!idPel || !sandi) return res.status(400).json({ error: 'ID Pelanggan dan sandi wajib diisi' });

        const pel = await queryOne(
            `SELECT id, nama, username, no_hp, email, alamat, tipe_koneksi, status, paket_id, password
             FROM pelanggan WHERE id=? LIMIT 1`, [idPel]
        );
        if (!pel) return res.status(404).json({ error: 'ID Pelanggan tidak ditemukan' });

        const digits = (s) => String(s || '').replace(/\D/g, '').replace(/^0/, '62');
        let ok = false;
        // 1) sandi = nomor WA pelanggan (default)
        if (pel.no_hp && digits(sandi) && digits(sandi) === digits(pel.no_hp)) ok = true;
        // 2) fallback: sandi bcrypt bila pelanggan sudah menggantinya sendiri
        if (!ok && pel.password) ok = await bcrypt.compare(sandi, pel.password).catch(() => false);
        if (!ok) return res.status(401).json({ error: 'Sandi salah' });

        delete pel.password;
        const token = jwt.sign(
            { id: pel.id, no_hp: digits(pel.no_hp), username: pel.username },
            JWT_SECRET + '_client',
            { expiresIn: '24h' }
        );
        res.json({ token, pelanggan: pel });
...
// ── Parse perangkat terhubung dari parameters ACS Lite (flat map) ──
// key: InternetGatewayDevice.LANDevice.1.Hosts.Host.<i>.<Field>  atau  Device.Hosts.Host.<i>.<Field>
function parseHostsAcslite(parameters) {
    if (!parameters || typeof parameters !== 'object') return [];
    const hosts = {};
    const re = /Hosts\.Host\.(\d+)\.([A-Za-z]+)$/;
    for (const key in parameters) {
        const m = key.match(re);
        if (!m) continue;
        (hosts[m[1]] = hosts[m[1]] || {})[m[2]] = parameters[key];
    }
    const out = [];
    for (const i in hosts) {
        const h = hosts[i];
        const mac = h.MACAddress || h.PhysAddress || '';
        const item = {
            hostname: h.HostName || '',
            ip:       h.IPAddress || '',
            mac:      String(mac).toUpperCase(),
            active:   /^(1|true)$/i.test(String(h.Active == null ? '' : h.Active)),
            iface:    h.InterfaceType || '',
            sumber:   h.AddressSource || ''
        };
        if (item.mac || item.ip || item.hostname) out.push(item);
    }
    return out;
...
router.get('/perangkat-wifi', clientAuth, async (req, res, next) => {
    try {
        ...
        if (dev) {
            ...
            if (req.query.manual === '1') { try { await genie.refreshDevice(dev.genie_id); } catch (e) {} }
            return res.json({ device: true, last_inform: dev.last_inform, perangkat, source: 'genieacs' });
        }

        // 2) Fallback ACS Lite (ONU yang sudah dipindah ke ACS Lite)
        const la = await cariDeviceAcslite(req.client.id).catch(() => null);
        if (la) {
            let perangkat = parseHostsAcslite(la.parameters);
            perangkat.sort((a, b) => (b.active - a.active) || ((a.hostname || a.ip || '').localeCompare(b.hostname || b.ip || '')));
            // ACS Lite hanya menyimpan whitelist parameter (tanpa subtree Hosts).
            // Bila memang tak ada data host, beri tahu portal agar pesannya akurat.
            const hostsSupported = perangkat.length > 0;
            return res.json({
                device: true, last_inform: la.last_inform, perangkat,
                source: 'acslite', hosts_unsupported: !hostsSupported
            });
        }

        return res.json({ device: false, perangkat: [], last_inform: null });
    } catch(e) { next(e); }
});
