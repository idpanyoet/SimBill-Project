...
router.post('/', async (req, res, next) => {
    ...
    try {
        const { nama, username, password, no_hp, email, alamat,
                ...
                no_ktp, tgl_lahir, ktp_url, reseller_id, siklus } = req.body;
        ...
        const result = await query(`
            INSERT INTO pelanggan (nama, username, password, no_hp, email, alamat,
                ...
                latitude, longitude, odc, odp, no_ktp, tgl_lahir, ktp_url, reseller_id, siklus,
...
router.put('/:id', async (req, res, next) => {
    try {
        const { nama, no_hp, email, alamat, paket_id, tipe_koneksi, notes, username, password, status, siklus } = req.body;
        if (!nama || !no_hp)
        ...
        await query(`
            UPDATE pelanggan SET nama=?, no_hp=?, email=?, alamat=?,
                paket_id=?, tipe_koneksi=?, notes=?, username=?, status=?, siklus=?,
                latitude=?, longitude=?, odc=?, odp=?,
