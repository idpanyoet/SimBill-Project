...
// Petakan satuan_masa paket ke unit dayjs (fix bug: dulu selalu 'day').
const _satuanUnit = (s) => (s === 'jam' ? 'hour' : (s === 'bulan' ? 'month' : 'day'));
...
router.post('/beli/user', resellerAuth, async (req, res, next) => {
    ...
    try {
        ...
        const hasil = await withTransaction(async (db) => {
            ...
            const tgl_expired = dayjs().add(paket.masa_aktif, _satuanUnit(paket.satuan_masa)).format('YYYY-MM-DD');
...
router.post('/pelanggan/:id/perpanjang', resellerAuth, async (req, res, next) => {
    try {
        ...
        const hasil = await withTransaction(async (db) => {
            ...
            const tglBaru = mulai.add(paket.masa_aktif, _satuanUnit(paket.satuan_masa)).format('YYYY-MM-DD');
    ...
    try {
        ...
        const inv = await queryOne(`
            SELECT i.*, p.username, p.reseller_id, p.tgl_expired, p.paket_id AS pel_paket_id,
                   p.tipe_koneksi, pk.masa_aktif, pk.satuan_masa, pk.nama AS nama_paket
            FROM invoice i
        ...
        const hasil = await withTransaction(async (db) => {
            ...
            if (inv.masa_aktif) {
                ...
                tglBaru = mulai.add(inv.masa_aktif, _satuanUnit(inv.satuan_masa)).format('YYYY-MM-DD');
...
async function prosesTopupWebhook(order_id, payment_type, paidAmount = null) {
    let hasil;
    try {
        hasil = await withTransaction(async (db) => {
            ...
            // GUARD NOMINAL (defense-in-depth, khususnya Xendit yg signature-nya
            // tak mengikat amount): tolak bila yang dibayar KURANG dari jumlah topup.
            if (paidAmount !== null && Math.round(Number(paidAmount)) < Math.round(Number(t.jumlah))) {
                console.warn(`[RESELLER] ⚠️ Topup ${order_id}: nominal bayar (${Math.round(Number(paidAmount))}) < jumlah (${Math.round(Number(t.jumlah))}) — TOLAK.`);
                return null;
            }
