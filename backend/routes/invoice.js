...
// Kolom pencatat petugas yang memproses pembayaran (untuk Laporan per Petugas)
(async () => {
  try {
    await query(`ALTER TABLE invoice ADD COLUMN IF NOT EXISTS dibayar_oleh VARCHAR(120) NULL`);
    await query(`ALTER TABLE invoice ADD COLUMN IF NOT EXISTS dibayar_oleh_id INT NULL`);
  } catch (e) { console.warn('[invoice] migrate dibayar_oleh:', e.message); }
})();
    ...
    try {
        ...
        if (invCek.pelanggan_id === null) {
            if (invCek.status === 'paid') {
                ...
                await query(`UPDATE invoice SET status='paid', tgl_bayar=NOW(), metode_bayar='tunai', dibayar_oleh=?, dibayar_oleh_id=? WHERE id=?`,
                    [req.admin?.nama || 'Admin', req.admin?.id || null, req.params.id]);
            }
            let kode = null;
            try {
        ...
        // Invoice PRORATA (bulan pertama prepaid+Fixed Date): JANGAN extend — tgl_expired
        // tetap di Fixed Date (= tgl_jatuh_tempo), karena prorata hanya menutup periode
        // parsial pertama. Perpanjangan penuh terjadi di invoice berikutnya.
        ...
        // KLAIM ATOMIK: cegah dobel-proses bila webhook gateway masuk barengan
        // (atau dua admin klik "Sudah Bayar" bersamaan). UPDATE ber-kondisi +
        // cek affectedRows → hanya SATU yang lolos; sisanya dibalas "sudah lunas".
        const claim = await query(
            `UPDATE invoice SET status='paid', tgl_bayar=NOW(), metode_bayar='tunai', dibayar_oleh=?, dibayar_oleh_id=? WHERE id=? AND status<>'paid'`,
            [req.admin?.nama || 'Admin', req.admin?.id || null, req.params.id]
        );
        if (!claim.affectedRows)
            return res.status(400).json({ error: 'Sudah lunas (diproses bersamaan)' });
        await query(
...
router.post('/:id/batalkan-lunas', async (req, res, next) => {
    try {
        ...
        await query(
            `UPDATE invoice SET status='unpaid', tgl_bayar=NULL, metode_bayar=NULL, dibayar_oleh=NULL, dibayar_oleh_id=NULL WHERE id=?`,
            [req.params.id]
        );
